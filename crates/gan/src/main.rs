...
// the scaffold passes the verdict it will be judged by: @moduledoc,
// @doc and @spec are what `gan build` asks of every public def
...
// The MCP surface is wired for the agents that read a project-level file
// (GEP-0028): the command is the same everywhere and carries no paths —
// `uv run` resolves this project's environment from wherever the client
// launches, `gan` finds gan-mcp in its .venv/bin, and the server walks up
// to the nearest gandora.jsonc. Check these in; they work for everyone.

/// Claude Code project scope: `.mcp.json` at the project root.
const INIT_MCP_CLAUDE: &str = r#"{
  "mcpServers": {
    "gandora": {
      "type": "stdio",
      "command": "uv",
...
/// Codex project scope: `.codex/config.toml`, read for trusted projects.
const INIT_MCP_CODEX: &str = r#"# Codex reads this for a trusted project; `codex mcp list` shows it.
[mcp_servers.gandora]
command = "uv"
args = ["run", "gan", "mcp"]
"#;

/// opencode project scope: `opencode.json` at the project root.
const INIT_MCP_OPENCODE: &str = r#"{
...
/// Writes the project-level MCP configuration each agent reads on its
/// own (GEP-0028-R012). Never overwrites a file the project already has.
fn write_mcp_configs(path: &Path) -> Result<(), Diagnostic> {
    write_if_absent(&path.join(".mcp.json"), INIT_MCP_CLAUDE)?;
    write_if_absent(&path.join("opencode.json"), INIT_MCP_OPENCODE)?;
    std::fs::create_dir_all(path.join(".codex")).map_err(io_err(path))?;
    write_if_absent(&path.join(".codex/config.toml"), INIT_MCP_CODEX)
}
...
fn cmd_init(args: &[String]) -> Result<(), Diagnostic> {
    ...
    // gan-mcp joins the dev group so `gan mcp` resolves from this project's
    // own .venv — the MCP configs below carry no paths because of it
