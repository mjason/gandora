...
def _find_q(*_gan_args):
    match _gan_args:
        case ([pattern, *rest] as _gan_l19, root,) if isinstance(_gan_l19, list):
            return _emit(gandora_lsp.retrieval.find(root, pattern, gandora_std.enum.member_p(rest, "--deps")))
        case ([] as _gan_l20, _root,) if isinstance(_gan_l20, list):
            return _usage_halt()
    raise GanMatchError("no clause of find_q/2 matched " + repr(_gan_args))


def _grep_q(*_gan_args):
    match _gan_args:
        case ([pattern, *rest] as _gan_l21, root,) if isinstance(_gan_l21, list):
            deps = gandora_std.enum.member_p(rest, "--deps")
            if _gan_truthy(gandora_std.enum.member_p(rest, "--ranked")):
                return _emit(gandora_lsp.retrieval.ranked(root, pattern, deps))
            else:
                return _emit(gandora_lsp.retrieval.grep(root, pattern, deps))
        case ([] as _gan_l22, _root,) if isinstance(_gan_l22, list):
            return _usage_halt()
    raise GanMatchError("no clause of grep_q/2 matched " + repr(_gan_args))


def _read_q(*_gan_args):
    match _gan_args:
        case ([target, *rest] as _gan_l23, root,) if isinstance(_gan_l23, list):
            span = _positional(rest)
            from__kw = _line_or_zero(gandora_std.enum.at(span, 0))
            to = _line_or_zero(gandora_std.enum.at(span, 1))
            return _emit(gandora_lsp.retrieval.read(root, target, from__kw, to))
        case ([] as _gan_l24, _root,) if isinstance(_gan_l24, list):
            return _usage_halt()
    raise GanMatchError("no clause of read_q/2 matched " + repr(_gan_args))


def _line_or_zero(arg):
    if (arg is None):
        return 0
    else:
        try:
            return builtins.int(arg)
        except Exception as _e__gan53:
            return 0


def _doc_q(*_gan_args):
    match _gan_args:
        case ([target, *rest] as _gan_l25, root,) if isinstance(_gan_l25, list):
        ...
        case ([] as _gan_l26, _root,) if isinstance(_gan_l26, list):
            return _usage_halt()
...
def _definition_q(*_gan_args):
    match _gan_args:
        case ([target, *_] as _gan_l27, root,) if isinstance(_gan_l27, list):
            return _emit(core.definition(target, root))
        case ([] as _gan_l28, _root,) if isinstance(_gan_l28, list):
            return _usage_halt()
...
def _symbols_q(*_gan_args):
    match _gan_args:
        case ([module, *rest] as _gan_l29, root,) if isinstance(_gan_l29, list):
        ...
        case ([] as _gan_l30, _root,) if isinstance(_gan_l30, list):
            return _usage_halt()
...
def _references_q(*_gan_args):
    match _gan_args:
        case ([target, *_] as _gan_l31, root,) if isinstance(_gan_l31, list):
            return _emit(core.references(target, root))
        case ([] as _gan_l32, _root,) if isinstance(_gan_l32, list):
            return _usage_halt()
...
def _wsymbols_q(*_gan_args):
    match _gan_args:
        case ([query, *_] as _gan_l33, root,) if isinstance(_gan_l33, list):
            return _emit(core.wsymbols(query, root))
        case ([] as _gan_l34, root,) if isinstance(_gan_l34, list):
...
def _check_q(rest, root):
    ...
    if _gan_truthy(gandora_std.enum.empty_p(errors0)):
        ...
        try:
            _gan_tmp36 = gandora_tool.verifier.verify(root, cache, core.build(root, cache), gandora_std.enum.member_p(rest, "--strict"))
        except Exception as _e__gan76:
            _gan_tmp36 = []
        verification = _gan_tmp36
        _gan_tmp35 = diags + verification
    else:
        _gan_tmp35 = diags
    diags = _gan_tmp35
    try:
        _gan_tmp37 = gandora_std.enum.uniq(gandora_std.enum.map(core.wsymbols("", root), lambda sym: gandora_std.map.get(sym, "path")))
    except Exception as _e__gan77:
        _gan_tmp37 = gandora_std.path.wildcard(gandora_std.path.join(root, "src/**/*.gan"))
    files = _gan_tmp37
    test_files = gandora_std.path.wildcard(gandora_std.path.join(root, "tests/*.gan"))
    def _gan_fn1(path, *, diags=diags, root=root):
        try:
            _gan_tmp38 = _read(path)
        except Exception as _e__gan78:
            _gan_tmp38 = ""
        text = _gan_tmp38
...
def _pydoc_q(*_gan_args):
    match _gan_args:
        case ([chain, *_] as _gan_l39, root,) if isinstance(_gan_l39, list):
            return _emit(gandora_lsp.py_intel.hover_markdown(root, import_line(chain), chain))
        case ([] as _gan_l40, _root,) if isinstance(_gan_l40, list):
            return _usage_halt()
...
def _pycomplete_q(*_gan_args):
    match _gan_args:
        case ([chain, *_] as _gan_l41, root,) if isinstance(_gan_l41, list):
            return _emit(gandora_lsp.py_intel.complete(root, import_line(chain), chain))
        case ([] as _gan_l42, _root,) if isinstance(_gan_l42, list):
            return _usage_halt()
...
def _pygoto_q(*_gan_args):
    match _gan_args:
        case ([chain, *_] as _gan_l43, root,) if isinstance(_gan_l43, list):
            return _emit(gandora_lsp.py_intel.goto(root, import_line(chain), chain))
        case ([] as _gan_l44, _root,) if isinstance(_gan_l44, list):
            return _usage_halt()
...
def _pysig_q(*_gan_args):
    match _gan_args:
        case ([chain, *_] as _gan_l45, root,) if isinstance(_gan_l45, list):
            return _emit(gandora_lsp.py_intel.signatures(root, import_line(chain), chain))
        case ([] as _gan_l46, _root,) if isinstance(_gan_l46, list):
            return _usage_halt()
...
def _doc_one(target, root, brief):
    ...
    if (info is None):
        _gan_tmp47 = _symbol_stub(target, root)
    else:
        _gan_tmp47 = info
    info = _gan_tmp47
...
def _symbol_stub(target, root):
    ...
    if gandora_std.enum.count(parts) != 2:
        ...
        try:
            _gan_tmp48 = core.symbols(mod, root)
        except Exception as _e__gan99:
            _gan_tmp48 = []
        syms = _gan_tmp48
        ...
        if (hit is None):
            ...
            _gan_fstr49 = gandora_std.map.get(hit, "kind")
            return {"label": target, "kind": gandora_std.map.get(hit, "kind"), "head": gandora_std.map.get(hit, "head"), "entries": {"default": f"(undocumented — add @doc above the {_gan_fstr49})"}}


def _project_doc(name, root):
    try:
        _gan_tmp50 = core.wsymbols(name, root)
    except Exception as _e__gan102:
        _gan_tmp50 = []
    syms = _gan_tmp50
    hit = gandora_std.enum.find(syms, lambda s, *, name=name: gandora_std.map.get(s, "name") == name)
    if (hit is None):
        ...
        _gan_fstr51 = gandora_std.map.get(hit, "module")
        return core.doc(f"{_gan_fstr51}.{name}", root)
...
def _brief_doc(target, info):
    ...
    if _gan_truthy(gandora_std.enum.empty_p(specs)):
        ...
        _gan_tmp52 = gandora_std.enum.at(specs, 0)
    head = _gan_tmp52
...
def _take_root(args):
    _gan_case53 = gandora_std.enum.find_index(args, lambda a: a == "--root")
    match _gan_case53:
        case None:
