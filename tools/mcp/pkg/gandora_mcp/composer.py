...
async def _constructs(requirement, root):
    ...
    wanted = _gan_tmp5
    names = gandora_std.enum.uniq(gandora_std.enum.concat(wanted))
    cards = (await gandora_std.task.async_stream(names, lambda n, *, root=root: gandora_std.task.blocking(lambda *, root=root: _card(n, root)), 4))
