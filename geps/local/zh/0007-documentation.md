...
**GEP-0007-R001:** `@doc` 和 `@moduledoc` 接受：一个字符串（默认文本）、原子-关键字形式 `default: "...", <locale>: "..."`（区域键使用 `_` 代替 `-` 拼写 BCP 47 标签，如 `zh_CN` ≡ `zh-CN`），或 `false`。文档文本为 Markdown；编译器原样存储，并且 MUST NOT 重新排版或重新格式化。
