...
编译为可读的 Python 代码而非字节码或解释器，使得整个 Python 生态系统——调试器、性能分析器、`uv`、部署目标——无需 Gandora 专有支持即可使用，可读输出加上 `.venv` 兼容性是最便宜的集成方案。

单文件对应单模块的规则（不同于 Elixir 允许一个文件包含多个模块）实现了从模块名称到生成的 Python 模块之间直接、可预测的映射，进而简化了导入、工具链和增量编译。路径派生的模块名称让这一映射完全机械化。
